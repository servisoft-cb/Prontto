...
uses
  ...
  ExtCtrls, Mask, JvToolEdit, JvTypedEdit, JvEdit, FMTBcd, DB, DBClient, Provider, SqlExpr, dbxpress, ComCtrls, UDm1,
  rsDBUtils;
  ...
  if JvDateEdit1.Date > 0 then
    begin
      try
        ...
        vId := Fdm1.Incrementa('NUMCRECEBER','CRECEBERPARC');
        vId := vId - 1;
              ...
              begin
                ...
                if not(tCReceberParc.IsEmpty) and (tCReceberParcVLRPAGTO.AsFloat <= 0) then
                  begin
                    ...
                    if StrToFloat(FormatFloat('0.00',Fdm1.tCReceberParcVLRVENCIMENTO.AsFloat)) <>
                       StrToFloat(FormatFloat('0.00',tCReceberParcVLRVENCIMENTO.AsFloat)) then
                      begin
                ...
                if tCReceberParc.IsEmpty then
                  begin
                    ...
                    Fdm1.tCReceberParcNUMCRECEBER.AsInteger   := vId;
                    Fdm1.tCReceberParcPARCELA.AsString        := '01';
                    Fdm1.tCReceberParcVLRVENCIMENTO.AsFloat   := StrToFloat(FormatFloat('0.00',qPessoaVLRPGTO.AsFloat + vValor));
                    Fdm1.tCReceberParcVLRRESTANTE.AsFloat     := StrToFloat(FormatFloat('0.00',qPessoaVLRPGTO.AsFloat + vValor));
                    Fdm1.tCReceberParcDTVENCIMENTO.AsDateTime := StrToDate(vData);
                    Fdm1.tCReceberParcCODBANCO.AsInteger      := qPessoaCODBANCO.AsInteger;
