...
uses
  ...
  JvDBCtrl, DBCtrls, UCBase, dbxpress, FMTBcd, DB, SqlExpr, JvCombobox, JvComponent, JvColorBox, SMDBGrid, UDm1, rsDBUtils;
...
uses uPessoa, UMenu, UDmCons, UPrevPessoa, URelPessoas, URelPessoasContrato, URelPessoaFicha,
  uImportaAlunos;
...
begin
  Fdm1.tPessoa.Close;
  vPessoa := Fdm1.ctPessoa + ' WHERE 0 = 0';

  if Edit2.Text <> '' then
  begin
     vPessoa :=  vPessoa + ' AND (P.FONE LIKE ''%' + Edit2.Text + '%'' OR P.FAX LIKE ''%' + Edit2.Text + '%'' OR P.CEL LIKE ''%' + Edit2.Text + '%'')';
  end
  else
  ...
  if not Fdm1.tPessoa.IsEmpty then
    ...
    begin
      try
        ...
        qDeleta.Close;
        qDeleta.SQL.Clear;
        qDeleta.SQL.Add('Delete from PESSOA_ATENDIMENTO where PESSOA_ID = ' + Fdm1.tPessoaID.AsString);
        qDeleta.ExecSQL();
        ...
        qDeleta.Close;
        qDeleta.SQL.Clear;
        qDeleta.SQL.Add('Delete from ALARME where CLIENTE_ID = ' + Fdm1.tPessoaID.AsString);
        qDeleta.ExecSQL();
...
begin
  ...
  begin
    ...
    if Fdm1.tPessoaSTATUS.AsString = '2' then
      Background  := clFuchsia;
    if fdm1.tPessoaID_INTERNET.AsInteger > 0 then
      AFont.Color := clBlue;
  end;
end;
...
begin
  ...
  fRelPessoaFicha.RlReport1.Preview;
